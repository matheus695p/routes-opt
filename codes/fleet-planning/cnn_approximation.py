...
model.add(tf.keras.layers.Conv1D(filters=32, kernel_size=8,
                                 activation='relu',
          input_shape=(x_cnn1d_train.shape[1], 1)))
...
# entrenar
history = model.fit(x=x_cnn1d_train, y=y_train,
...
results = model.evaluate(x_cnn1d_test,  y_test,
                         batch_size=args.batch_size)

# predictions
y_pred = model.predict(x=x_cnn1d_test)
# desconvertir
