...
model.add(tf.keras.layers.Dense(1024, input_dim=x_nn_train.shape[1]))
...
# entrenar
history = model.fit(x=x_nn_train, y=y_train,
...
results = model.evaluate(x_nn_test,  y_test,
                         batch_size=args.batch_size)

# predictions
y_pred = model.predict(x=x_nn_test)
# desconvertir
...
model.save("models/time_travel_model.h5")
